...
- Server-only secrets: YouCam, OAuth, Postgres, object storage, malware scanner, worker, cron, receipt signer.
...
- Retailer OAuth with an exact email allowlist and organization-derived tenant identifier.
...
- Private S3-compatible storage with SSE-KMS, opaque keys, SHA-256 evidence, deletion followed by absence verification.
...
- Configure MFA and least-privilege roles in hosting, database, storage, GitHub, OAuth, YouCam, scanner, and monitoring accounts.
